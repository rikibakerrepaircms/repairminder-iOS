...
| `Core/Networking/APIEndpoints.swift` | Add 10 new POS endpoint cases |
...
Also extend `OrderDeviceSummary` with 3 new optional fields needed by Stage 4 (Buyback Payout):

```swift
// Add to existing OrderDeviceSummary struct
let displayName: String?      // Not in current API — requires backend update (see below)
let serialNumber: String?     // Not in current API — requires backend update (see below)
let payoutAmount: Double?     // Not in current API — requires backend update (see below)
```

These fields are optional so the app will compile and work without the backend change. Stage 4's `BuybackPayoutSheet` handles nil gracefully (shows "Unknown Device" for missing `displayName`, hides serial if nil, shows 0 for missing `payoutAmount`).
...
case refundTerminalPayment(transactionId: String)           // POST /api/pos/payments/:id/refund
...
case .refundTerminalPayment(let id): return "/api/pos/payments/\(id)/refund"
...
case .initiateTerminalPayment, .createPaymentLink, .cancelTerminalPayment,
     .cancelPaymentLink, .resendPaymentLinkEmail, .refundTerminalPayment:
    return .post
```
...
## Backend Change Required (Pre-Stage 4)

The order detail endpoint's device summary query in `worker/order_handlers.js` (around line 1254) currently returns only: `id`, `status`, `workflow_type`, `ready_for_collection_at`, `authorization_status`, `authorization_method`, `authorized_at`. It needs extending to include:

- **`display_name`** — requires a JOIN: `COALESCE(d.custom_brand, db.name, '') || ' ' || COALESCE(d.custom_model, dm.name, 'Unknown Device') AS display_name`
- **`serial_number`** — from `d.serial_number` (direct column)
- **`imei`** — from `d.imei` (direct column, fallback for serial)
- **`payout_amount`** — from `d.authorisation_amount` (the agreed buyback price)

This is a backend-only change (single query modification). It must land before Stage 4 can be fully tested, but Stage 4's Swift code can be written in parallel since all three fields are declared as optionals in `OrderDeviceSummary`.
...
- [ ] `PosModels.swift` created with: `PosIntegration`, `PosIntegrationConfig`, `PosTerminal`, `PosTransactionStatus`, `PosTransactionPollResponse`, `InitiateTerminalPaymentRequest`, `InitiateTerminalPaymentResponse`, `PosPaymentLink`, `PaymentLinkStatus`, `CreatePaymentLinkRequest`, `CreatePaymentLinkResponse`
- [ ] `ManualPaymentRequest` added to `Order.swift`
- [ ] `DevicePaymentBreakdown` added to `Order.swift`
- [ ] 10 new cases added to `APIEndpoints.swift` with correct paths, methods, and query items
- [ ] `PaymentService.swift` created with 11 methods
- [ ] `OrderDetailViewModel` extended with payment state properties, 5 methods and 4 computed properties
