...
**Web-parity follow-up (merge `9f2f721`, pushed):** closed the remaining gaps — **replacement-asset linking on Resolve** (`ResolveReplacementSheet`: search in-stock by tag/name → Link & Resolve / Resolve Without Link, mirrors web `ReplacementLinkModal`), **edit now clears condition_grade** (sends `""`), **copy asset-tag** button on detail, **Return-to-Supplier disabled-reason** surfaced in the menu, and a testable **`AssetActions`** gating helper. Tests now **34** (added gating + all remaining VM mutations + replacement/clear encoding). **7/8 mutations runtime-verified on prod** (edit/move/delete/return-to-supplier/resolve-with-replacement/deploy-external/return-external); **allocate NOT driven live** (creating a throwaway order needs client+location+signature AND there is no order-delete endpoint → would permanently pollute prod) — it's covered by unit request-encoding + response-decoding-vs-real-contract + the shared `requestFull` mechanism (live-proven via edit). Only Dymo label-printing is intentionally skipped (iOS can't print to Dymo). **Not runtime-driven:** the SwiftUI UI itself has no XCUITest (demo account has zero assets); gating is unit-tested, wiring is build/code-verified.
